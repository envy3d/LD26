...
public abstract class RPlace {
	...
	public RPlace(int x, int y, int width, int height, int maxAntsInside, int numOfJunctions, GameScreen gameScreen, boolean isEnterable, boolean isExitable, int type) {
		...
		accesses = new RLoc[numOfJunctions];
		...
		if (numOfJunctions <= 4) {
			...
			for (int i = 0; i < filledSides.length; i++) {
				if (filledSides[i] == 1) {
					...
					accesses[currentJunctionIdx] = game.map.addRLoc(rand, top + 1);
				}
				else if (filledSides[i] == 2) {
					rand = MathUtils.random(bottom + 1, top - 1);
					accesses[currentJunctionIdx] = game.map.addRLoc(left - 1, rand);
				}
				else if (filledSides[i] == 3) {
					rand = MathUtils.random(left + 1, right - 1);
					accesses[currentJunctionIdx] = game.map.addRLoc(rand, bottom - 1);
				}
				else if (filledSides[i] == 4) {
					rand = MathUtils.random(bottom + 1, top - 1);
					accesses[currentJunctionIdx] = game.map.addRLoc(right + 1, rand);
				}
				currentJunctionIdx++;
			}
		}
